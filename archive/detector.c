...
void save_detections(
    ...
    char fnout[256] = {};
    sprintf(fnout, "%s/%d.txt", root, idx);
    printf("fout : %s\n", fnout);
    FILE* fp = fopen(fnout, "w+");
            fprintf(fp, "%s\n", fid);
            fprintf(fp, "cx cy w h p\n");
        ...
        for(i=0; i < nboxes; ++i){
            ...
            for(j=0; j < nclass; ++j){
                if(det[i].prob[j] < pmax) continue;
                pmax = det[i].prob[j];
                jmax = j;
            }
            if (pmax < 0.2) continue;
            printf("%f\n", pmax);

            box b = det[i].bbox;
            fprintf(fp, "%f %f %f %f %f\n", b.x, b.y, b.w, b.h, det[i].prob[jmax]);
        }
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    if(isdir){
        strncpy(dirname, filename, 256);
        droot = opendir(dirname);
    }

    int idx = 0;
    while(1){
        if(isdir){
            subfile = readdir(droot);
            if (subfile == NULL) break;
            if (strcmp(subfile->d_name, ".") == 0) continue;
            if (strcmp(subfile->d_name, "..") == 0) continue;
            // filename = dirname + subfile->d_name
            strncpy(filename, dirname, 256);
        ...
        if(isdir){
            save_detections("/tmp/det", input, idx, dets, nboxes, l.classes);
            ++idx;
        }
